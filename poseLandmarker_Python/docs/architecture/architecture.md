...
- 런타임 의존성은 저장소 내부 포크 대신 패키지 매니저가 설치한 `opencv-python-headless`, `mediapipe`를 기준으로 유지한다.
- `third_party/`는 현재 저장소가 직접 소유해야 하는 외부 소스가 있을 때만 사용한다.
- 외부 오픈소스 내부 구현을 다시 확인해야 하면 필요 시 별도 로컬 clone 또는 upstream 저장소를 다시 내려받아 참조한다.
- 문서에는 특정 개발자 로컬 절대 경로를 기록하지 않는다.
...
- OpenCV 공개 API 선언은 설치된 OpenCV 패키지 헤더 또는 upstream OpenCV 저장소의 `modules/videoio/include/opencv2/videoio.hpp`를 기준으로 확인한다.
- 프레임 단위 위치 제어 상수: `CAP_PROP_POS_FRAMES`
- OpenCV `VideoCapture` 구현 진입점은 upstream OpenCV 저장소의 `modules/videoio/src/cap.cpp`를 기준으로 확인한다.
...
- Python 태스크 래퍼: upstream MediaPipe 저장소의 `mediapipe/tasks/python/vision/pose_landmarker.py`
- C++ 태스크 구현: upstream MediaPipe 저장소의 `mediapipe/tasks/cc/vision/pose_landmarker/pose_landmarker.cc`
- 내부 그래프 정의: upstream MediaPipe 저장소의 `mediapipe/tasks/cc/vision/pose_landmarker/pose_landmarker_graph.cc`
