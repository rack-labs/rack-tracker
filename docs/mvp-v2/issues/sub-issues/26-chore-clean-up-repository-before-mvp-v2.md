...
> docs/ 진입점 갱신, v1/v2 단계 경계 명시, 구 경로 참조 정리
...
> MVP-v0/ → archive/MVP-v0/ 이동, 현재 활성 경로에서 분리
...
> AGENTS.md 추가, 에이전트 작업 규칙과 README 역할 분리
...
> Codex 전용 표현 제거, 에이전트 공통 git workflow 문서로 일반화
...
> 브랜치 형식 type/issue → issue-number-type-desc 변경, 관련 문서 동기화
...
> GitHub Issue 생성 전 번호 추정 금지, placeholder 규칙 전 문서 적용
...
> AGENTS.md를 진입 허브로 축소, 세부 규칙을 docs/agent-workflow/로 분리
...
> 관련 workflow 문서 상단에 문서 관계 및 동기화 의무 헤더 추가
...
> 작업 시작 시 GitHub 이슈 확인 + 로컬 관리 문서 탐색 절차 연결
...
> poseLandmarker_JavaScript/ → archive/ 이동, 루트 README 활성 경로 정리
...
> .gitmodules 삭제, third-party-policy.md 추가, 절대경로 참조 제거
...
> docs/mvp-v1/features/mediapipe/를 core/issues/exploration/으로 역할 분리
...
> docs/mpv-v2/ → docs/mvp-v2/ 디렉터리 및 전체 참조 수정
...
> pre-execution gate 강화, 검증 스크립트 추가, 크기 예외 금지 문구 통일
...
> AGENTS.md에 pre-commit review gate 추가, CLAUDE.md 중복 제거
...
### docs: add repo commit type to collaboration convention (#26)

> git-collaboration-convention.md Commit Types 테이블에 repo 타입 추가
...
- `docs/agent-workflow/git-collaboration-convention.md`의 Commit Types 테이블에 `repo` 타입을 추가했다.
- 설명: `Repository structure changes (archiving, moving, removing directories or submodules)`
- 이 브랜치에서 사용된 `repo:` 커밋들이 정의되지 않은 타입 없이 컨벤션을 준수하도록 기준을 확립했다.

#### 검증

- 테이블에 `repo` 항목이 추가된 상태를 확인했다.

---

### docs: restructure management document format (#26)

> <details> 제거, 3구역 분리(---), ### 로그 항목 + > 한 줄 요약 + #### 서브섹션 구조 적용

#### scope

- remove `<details>` legacy wrapper from work log section
- apply consistent three-zone layout with `---` dividers
- use `###` per log entry, `>` one-line summary, `####` sub-sections
- update Issue Management Document Template in `docs/agent-workflow/templates.md` to match

#### changes

- rewrote `docs/mvp-v2/issues/sub-issues/26-chore-clean-up-repository-before-mvp-v2.md` with new structure
- updated Issue Management Document Template in `docs/agent-workflow/templates.md`

#### verification

- confirmed `<details>` tags are gone
- confirmed `---` separates the three zones
- confirmed all log entries use `###` + `>` + `####` structure consistently

#### note

- content of all prior log entries is preserved unchanged; only formatting was updated

### docs: enrich backend architecture section with slide images and detailed content (#26)

> README 백엔드 아키텍처 섹션 보강 — 슬라이드 이미지 삽입, Job State Machine, Polling 규약, 팀 역할 매트릭스 추가

#### 작업 단위

- 백엔드 아키텍처 섹션 전면 재작성
- PPTX 슬라이드 이미지 추출 및 `docs/etc/architecture-slides/`에 저장
- README에 슬라이드 캡처 이미지 삽입

#### 반영 내용

- `docs/etc/architecture-slides/` 아래 세 PPTX(`Motion_Analysis_Backend_Blueprint`, `MediaPipe_Pose_Blueprint`, `OpenCV_Frame_Extraction_Blueprint`)에서 이미지 추출했다.
- 6-Layer 아키텍처 스택, Job State Machine, Polling 통신 규약, 3분할 데이터 계약, 팀 역할 매트릭스 섹션을 추가했다.
- 각 섹션에 해당 슬라이드 이미지를 참고 이미지로 삽입했다.
- 슬라이드 내용 중 현재 코드와 다른 부분은 반영하지 않고 실제 코드 기준으로 작성했다.

#### 검증

- `README.md` 변경 내용을 직접 확인했다.

---

### docs: rewrite root README with team profile and backend architecture (#26)

> 루트 README.md 전면 재작성 — 팀 구성, 협업 프로세스, 백엔드 아키텍처 추가
...
- 루트 `README.md`를 프로젝트 소개용 문서로 전면 재작성했다.
- 팀원 소개 테이블(프로필 사진, 이름, 역할), 협업 프로세스(브랜치 전략, 커밋 컨벤션, PR 프로세스), 저장소 구조 섹션을 추가했다.
- 백엔드 아키텍처 섹션을 추가하고 uv/FastAPI, OpenCV, MediaPipe, 데이터 분석 파이프라인 네 영역으로 분리해 Mermaid 다이어그램과 역할 테이블로 시각화했다.
